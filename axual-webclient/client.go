...
func (c *Client) RequestAndMapGrant(method string, url string, reqBody io.Reader, header map[string]string, m interface{}) error {
	req, err := http.NewRequest(method, url, reqBody)
	...
	body, err := c.doRequestAppGrant(req)
	if err != nil {
		log.Println("Error:", err)
		return err
	}
...
func (c *Client) doRequestAppGrant(req *http.Request) ([]byte, error) {
	req.Header.Set("Realm", c.Realm)
